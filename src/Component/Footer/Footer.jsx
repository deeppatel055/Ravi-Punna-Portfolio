...
const Footer = () => {
    ...
    useEffect(() => {
        const observer = new IntersectionObserver(
            ...
            },
            {
                ...
                rootMargin: '0px 0px -50px 0px' // Trigger slightly before the footer is fully visible
            }
        );
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Create form data manually since we can't use form element
    ...
    return (
        <div >
            ...
            <footer
                ref={footerRef}
                className={`mt-8 mb-4 px-4 sm:px-6 md:mt-16 md:mb-7 lg:mt-20 max-w-7xl mx-auto transition-all duration-1000 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
                    }`}
            >
                <div className='flex flex-col xl:flex-row justify-between gap-6 sm:gap-8 lg:gap-12 xl:gap-16 items-start xl:items-center'>
                    ...
                    <div className={`flex-1 w-full xl:w-auto transition-all duration-1200 ease-out ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-16'
                        ...
                        <div className="mt-4 sm:mt-6 space-y-3 sm:space-y-4">
                            {contactInfo.map((info, index) => (
                                <div
                                    key={index}
                                    className={`flex items-center gap-3 sm:gap-4 transition-all duration-800 ease-out ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'
                                        }`}
                                    style={{ transitionDelay: `${500 + index * 150}ms` }}
                                >
                                    <div className="flex-shrink-0">
                                        <info.icon className="text-[#2d3748] w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 transition-transform duration-300 hover:scale-110" />
                                    </div>
                        ...
                        {/* Social Links */}
                        <div className={`mt-4 sm:mt-6 flex gap-3 sm:gap-4 transition-all duration-800 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                            ...
                            {socialLink.map((sl, index) => (
                                <a
                                    ...
                                    className={`text-[#2d3748] hover:text-[#4a5568] transition-all duration-300 p-1 hover:scale-110 transform ${isVisible ? 'opacity-100 translate-y-0 rotate-0' : 'opacity-0 translate-y-4 rotate-12'
                                        }`}
                                    style={{ transitionDelay: `${1100 + index * 100}ms` }}
                                >
                                    <sl.icon className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7" />
                                </a>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className={`flex-1 w-full xl:w-auto max-w-full sm:max-w-md lg:max-w-lg xl:max-w-xl transition-all duration-1200 ease-out ${isVisible ? 'opacity-100 translate-x-0 scale-100' : 'opacity-0 translate-x-16 scale-95'
